...
```{r}
attendance |> filter(Conference %in% powerfive) |> select(Institution, Conference, `2023`)
```
...
dllink <- download_link(
  link = "https://dwillis.github.io/sports-data-files/players24.csv",
  button_label = "Download csv file",
  button_type = "danger",
  has_icon = TRUE,
  icon = "fa fa-save",
...
**ANSWER** Who do you get as the first result, and what was his trueshooting percentage? TJ NeSmith has the highest trueshooting percentage, at 78.52%
...
Let's return to our women's soccer data. I've updated the data through Sept. 1, so we've got more matches now. Let's load it:
...
Now it's your turn: do the same for the 2023 data. Load it from GitHub, then adapt the above code but add an additional filter to restrict the results to Maryland's first five games (the 5th game was on Aug. 21, 2023). You need to put the date value in quotes.
...
```{r}
maryland_first_5_2023 <- matches_2023 |>
  filter(team == 'Maryland Terrapins, Terps') |>
...
**ANSWER**: 53.6 percent of Maryland's shots were on goal, which was a higher mark than in 2022.
